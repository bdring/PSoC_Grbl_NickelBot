...
void st_go_idle()
{
  ...
  Stepper_Timer_Stop(); // PSoc Change
  busy = false;
  ...
  Control_Step_Enable_Write(pin_state); // PSoc Change
...
void stepper_timer_isr() //ISR(TIMER1_COMPA_vect)
{
  Stepper_Timer_ReadStatusRegister(); // clear the PSoc interrupt
  
  if (busy) { return; } // The busy-flag is used to avoid reentering this interrupt

  // Set the direction pins a couple of nanoseconds before we step the steppers
   Control_Reg_Dir_Write(st.dir_outbits);
  ...
  busy = true;
  Stepper_Timer_Start(); //sei(); // Re-enable interrupts to allow Stepper Port Reset Interrupt to fire on-time.
  ...
  if (st.exec_segment == NULL) {
    ...
    if (segment_buffer_head != segment_buffer_tail) {
      ...
      Stepper_Timer_WritePeriod(st.exec_segment->cycles_per_tick); // OCR1A = st.exec_segment->cycles_per_tick;
...
void st_reset()
{
  ...
  Control_Reg_Step_Write(0b111 | step_port_invert_mask);
  //STEP_PORT = (STEP_PORT & ~STEP_MASK) | step_port_invert_mask;
  Control_Reg_Dir_Write(0b111 | dir_port_invert_mask);
...
void stepper_init()
{
  Clock_Step_Pulse_Start();
  isr_Step_StartEx(stepper_timer_isr);
}
